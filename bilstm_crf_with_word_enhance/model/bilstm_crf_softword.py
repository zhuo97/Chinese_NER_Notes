...
class BiLSTMSoftWord(nn.Module):
    def __init__(self,
                 ...
                 out_size,
                 word_enhance_size,
                 pretrain_embedding_weights):
        super(BiLSTMSoftWord, self).__init__()

        self.embedding = nn.Embedding.from_pretrained(pretrain_embedding_weights, False)
        self.softword_embedding = nn.Embedding(word_enhance_size, pretrain_embedding_weights.shape[-1])

        self.bilstm = nn.LSTM(input_size=pretrain_embedding_weights.shape[-1]*2,
...
class BiLSTM_CRF_SoftWord(nn.Module):
    def __init__(self,
        ...
        self.bilstm = BiLSTMSoftWord(hidden_size=hidden_size,
                                     ...
                                     pretrain_embedding_weights=pretrain_embedding_weights)
