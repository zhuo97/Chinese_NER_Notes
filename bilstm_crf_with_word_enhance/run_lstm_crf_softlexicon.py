...
from model.bilstm_crf_softlexicon import BiLSTM_CRF_SoftLexicon
from data_processor import MSRAProcessor
from vocabulary import Vocabulary
from word_enhance import SoftLexiconVocab, Soft2Idx, MaxLexiconLen
from datasets import NERDatasetSoftLexicon
...
def load_and_cache_dataset(args, processor, char_vocab, soft_lexicon_vocab, data_type="train"):
    ...
    if cached_dataset_file.exists():
        ...
        dataset = NERDatasetSoftLexicon(samples, char_vocab, soft_lexicon_vocab, args.label2id, args.device)
        logger.info("Saving features into cached file %s", cached_dataset_file)
        save_pickle(dataset, str(cached_dataset_file))
    return dataset


def train(args, model, processor, char_vocab, soft_lexicon_vocab):
    train_dataset = load_and_cache_dataset(args, processor, char_vocab, soft_lexicon_vocab, data_type="train")
    ...
    for epoch in range(1, 1 + args.epochs):
        ...
        for step, batch_samples in enumerate(train_loader):
            batch_word_ids, lengths, batch_softlexicon_ids, batch_softlexicon_weights, batch_label_ids = batch_samples
            batch_word_ids = batch_word_ids.to(args.device)
            batch_softlexicon_ids = batch_softlexicon_ids.to(args.device)
            batch_softlexicon_weights = batch_softlexicon_weights.to(args.device)
            batch_label_ids = batch_label_ids.to(args.device)
            loss = model(batch_word_ids, lengths, batch_softlexicon_ids, batch_softlexicon_weights, batch_label_ids)
            loss.backward()
        ...
        eval_log, class_info = evaluate(args, model, processor, char_vocab, soft_lexicon_vocab)
...
def evaluate(args, model, processor, char_vocab, soft_lexicon_vocab):
    eval_dataset = load_and_cache_dataset(args, processor, char_vocab, soft_lexicon_vocab, data_type="test")
    ...
    with torch.no_grad():
        for step, batch_samples in enumerate(eval_dataloader):
            ...
            predicted_labels = model.predict(batch_word_ids, lengths, batch_softlexicon_ids, batch_softlexicon_weights)
...
def main():
    ...
    parser.add_argument("--arch", default="bilstm_crf_softlexicon", type=str)
    # lr=0.0015 for msra
    ...
    char_vocab = Vocabulary(config.pretrain_char_emb_path)
    soft_lexicon_vocab = SoftLexiconVocab(config.pretrain_word_emb_path, config.msra_data_dir / "train")
    ...
    model = BiLSTM_CRF_SoftLexicon(
        hidden_size=args.hidden_size,
        label2id=args.label2id,
        device=args.device,
        word_enhance_size=len(Soft2Idx)-1, # 注意这里需要-1
        max_lexicon_length=MaxLexiconLen,
        char_embedding_weights=torch.tensor(char_vocab.pretrain_embeddings, dtype=torch.float32),
        word_embedding_weights=torch.tensor(soft_lexicon_vocab.soft_lexicon_embeddings, dtype=torch.float32)
    )

    model.to(args.device)
    if args.do_train:
        train(args, model, processor, char_vocab, soft_lexicon_vocab)
    if args.do_eval:
        model_path = args.output_dir / "best-model.bin"
