...
def load_and_cache_dataset(args, processor, vocab, data_type="train"):
    ...
    if cached_dataset_file.exists():
        ...
        dataset = NERDataset(samples, vocab, args.label2id, args.device)
...
def train(args, model, processor, vocab):
    ...
    for epoch in range(1, 1 + args.epochs):
        ...
        for step, batch_samples in enumerate(train_loader):
            batch_word_ids, lengths, batch_label_ids = batch_samples
            batch_word_ids = batch_word_ids.to(args.device)
            batch_label_ids = batch_label_ids.to(args.device)
            loss = model(batch_word_ids, lengths, batch_label_ids)
            loss.backward()

            # 使用对抗训练
            if args.use_adv_train:
                fgm.attack(epsilon=1, emb_name="embedding")
                loss_adv = model(batch_word_ids, lengths, batch_label_ids)
                loss_adv.backward()
...
def evaluate(args, model, processor, vocab):
    ...
    with torch.no_grad():
        for step, batch_samples in enumerate(eval_dataloader):
            ...
            predicted_labels = model.predict(batch_word_ids, lengths)
...
def main():
    ...
    parser.add_argument("--use_pretrain_embeddings", default=True, action="store_true")
    parser.add_argument("--use_adv_train", default=True, action="store_true")
    ...
    if args.use_pretrain_embeddings:
        vocab = Vocabulary(config.pretrain_char_emb_path)
    else:
        vocab = Vocabulary(config.pretrain_char_emb_path,
                           random_init=True,
                           random_init_emb_size=config.random_init_emb_size)
    ...
    model = BiLSTM_CRF(
        embedding_size=vocab.pretrain_embeddings_size,
        hidden_size=args.hidden_size,
        label2id=args.label2id,
        device=args.device,
        pretrain_embeddings=torch.tensor(vocab.pretrain_embeddings, dtype=torch.float32)
    )
